...
def test(args):
    model = model_selection(modelname='clip-vitb-32', num_out_classes=args.num_out)  # self.last_layer 2048-->num_out
    ...
    if args.dataset == 'DFDC':
        ...
        _, data_loader_dp = set_dataset_singleGPU_Deeper(preprocess=transforms,
                                                         ...
                                                         # '/data/DF-1.0/',#'/data0/mian2/DeeperForensics/',
...
def train(args):
    ...
    model = model_selection(modelname='clip-vitb-32', num_out_classes=args.num_out)
    model = model.cuda()
    model_without_ddp = model
